...
ユーザーの雑な指示（Issue化されていない）
  ...
  → gh issue create（auto-pickup ラベル付き。スコープが大きい/曖昧なら人間確認待ちでラベル無し）
...
| タイミング | 通知内容（例） |
|-----------|----------------|
| issue-planner がスコープ大/曖昧と判断し auto-pickup を付けなかった | `Issue #<N> を起票（要確認、auto-pickup未付与） — <title>` |
...
ユーザー: 「〇〇の一覧表示が遅いから直して」（雑な指示）
  ...
  → gh issue create（スコープが妥当なら auto-pickup ラベル付き）
  → （auto-pickup が付けば）issue-picker が後続を自動で拾う
```
...
5. 以下のテンプレートで Issue 本文を組み立て、`gh issue create --title "..." --body-file <tmp>` で作成する
6. **スコープ判定**（重要、6章の安全装置と直結）:
   - 影響範囲が限定的・実装方針が一意に決まる → `auto-pickup` ラベルを付ける
   - 破壊的変更・認証/決済/インフラに関わる・実装方針が複数考えられ一意に決めきれない → `auto-pickup` を**付けない**。Issue本文に「要確認: なぜ自動着手させなかったか」を明記し、`PushNotification` でユーザーに報告するのみで終了
...
- スコープ判断に自信が持てないのに安易に `auto-pickup` を付けない。付けるかどうかは可逆だが、誤って付けて自動実装が走ってしまう方が手戻りが大きい
