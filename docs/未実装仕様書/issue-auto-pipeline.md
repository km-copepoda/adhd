...
- **反復上限は2種類ある**: (a) 1件の指摘を `implementer`⇄`code-reviewer` で収束させる**内部上限（3回、固定）**、(b) PR全体でCodexへ再レビューを依頼する回数の**外側の上限**（`/codex-followup` の `iteration marker` 数。初期値3回から運用しながら10→15→20回まで引き上げ済み、2026-08-11）。どちらの上限に達しても諦めて人間にエスカレーションする。無限に反復しない
- 外側の上限に達した時点でまだ未解決の指摘が残っている場合、`/issue-picker` 由来のPRなら紐づくIssueを `auto:blocked` に遷移させ通知する（遷移させないと、PRは`auto:pr-open`のまま・`issue-picker`は次回もこのIssueを候補から除外し続け、人間への通知なしに永久停止する）
...
- [x] 外側のレビュー反復上限は当初3回で設計したが、運用しながら10→15→20回まで引き上げ済み（2026-08-11、ユーザー指示）。内部上限（1指摘あたりのimplementer⇄code-reviewer収束、3回固定）とは別物なので混同しないこと
...
3. **カテゴリに関わらず、対象がテスト可能なアプリケーションコードなら `test-writer` を呼んで Red を追加してから進める**（CLAUDE.md の TDD 規約通り。UI/ロジックカテゴリだからといって既存テストのGreen維持確認だけで済ませない — 回帰テストが無いまま承認すると同じ不具合が再発しても検知できない）。対象がドキュメント/設定ファイルのみ（`.claude/agents/*.md` 等、関数・モジュール・APIを持たない）の場合のみ `test-writer` を省略し「テスト対象外モード」で `implementer`/`code-reviewer` を呼ぶ（実装済み、2026-08-11。当初案の「QAカテゴリのみ必須」からTDD原則優先へ変更した）
