...
- [2026-05-11: 親画面に「子供モード（child-view）」を導入（親が子供端末を持たない家庭向けの代理操作）](#2026-05-11-親画面に子供モードchild-viewを導入親が子供端末を持たない家庭向けの代理操作)
...
- [2026-05-26: 親画面の carryOver 放置バッジを「N日間未完了」→「N回未完了」（出現回数ベース）に変更](#2026-05-26-親画面の-carryover-放置バッジをn日間未完了→n回未完了出現回数ベースに変更)
