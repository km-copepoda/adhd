...
## 2026-04-16: 進化確率の下限（MIN_EVOLUTION_PROBABILITY = 10%）を導入

### 決定内容
- `computeEvolutionWeights` に `MIN_EVOLUTION_PROBABILITY = 0.10` を導入
- アルゴリズム: 各パスに10%を保証 + 残り70%を実績比率で配分
  - 例: studyPt=5, staminaPt=0, lifePt=5 → STUDY=45%, STAMINA=10%, LIFE=45%
- 結果として支配パスの最大確率も 60% → 52%（0.6×0.7+0.1）に変化

### 理由
- スタミナタスクはカテゴリとして登録しづらく、0ptのまま進化を迎えるとSTAMINA系モンスターへの進化確率が0%になる
- 0%のパスが存在するとADHD特性の子供は「スタミナモンスターには絶対なれない」と感じ、カテゴリへの関心を失う
- 10%の下限を設けることで「何もやらなくても少し可能性がある」体験を維持し、タスク多様性への参加障壁を下げる
