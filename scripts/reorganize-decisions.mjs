...
const SUPERSEDED = {
  ...
  "2026-05-02: ひろば「エールを送る」スタンプ機能の導入": {
    ...
    kind: "partial",
    note: "「グループ全員に Web Push 配信」契約は 2026-05-05 で `DONE` 受信者にはスキップするように変更。「掲示板への書き込み禁止」条項は 2026-05-07 で撤回。エール送信機能本体は現行。",
  },
  ...
  "2026-05-29: 宝箱抽選を「レア度ごと独立抽選 + プールから均等選択」に変更": {
    ...
    kind: "partial",
    note: "「レア度ごと独立抽選 + 加算」部分は 2026-05-30 で排他的単発抽選に置き換え（1タスクで複数レア度当選を解消）。ただし「選択したレア度でプールが空だった場合の downgrade（RARE→UNCOMMON→COMMON）」ルールは 2026-05-30 でも明示的に維持されており、現行契約。",
  },
  ...
  "2026-06-03: 親ごほうび当選確率を引き下げ (COMMON 1/7→1/10 / UNCOMMON 1/14→1/20 / RARE 1/28→1/30)": {
    ...
    kind: "partial",
    note: "RARE 当選確率のみ同日中に 1/30→1/45 に再調整（COMMON / UNCOMMON は現行値）。「コレクション報酬で外れ救済不要のため pity は再導入しない」宣言は 2026-06-24 で撤回、pity 10 連続閾値で復活。",
  },
  ...
  // NOTE: `2026-03-28: 転生後の卵の孵化閾値を5ptに設定` は現行仕様のまま。
  // 2026-04-04 の卵選択ボーナスは進化パス選択確率に効くもので、孵化閾値
  // (REBIRTH_EGG_THRESHOLD = 5) は変更していない（Codex 指摘で誤った撤回
  // マークを取り下げ）。
