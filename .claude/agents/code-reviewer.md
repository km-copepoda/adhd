...
### 1. TDD 準拠
- [ ] `src/__tests__/` に新規/更新テストがあるか（**対象が `.gitignore`・`docs/`・`.claude/agents/*.md`・`.claude/commands/*.md` 等、関数・モジュール・APIを持たないドキュメント/設定ファイルのみの変更の場合はこの項目は該当なしとし、指摘しない**）
- [ ] `npm test` がグリーンか（テスト対象外の変更は「該当なし」でよい）
- [ ] 境界値テストが含まれているか（同上、テスト対象外の変更には適用しない）
...
- テストが FAIL のまま APPROVED を出さない（テスト対象外のドキュメント/設定ファイルのみの変更で「該当なし」の場合を除く）。
