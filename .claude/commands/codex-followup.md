...
- **コード修正が必要**:
  ...
  - 衝突なし → **指摘のカテゴリ判定**（下表）を行い、対応するコンテキストを付与した上で **`$headBranch` 上で** **`test-writer` (Red)** → **`implementer` (Green + Refactor)** → **`code-reviewer`** の順でサブエージェントを呼ぶ（コミット・pushのタイミングは後述）
    ...
    **作業場所の決め方（この反復で処理する全指摘に共通、指摘ごとに作り直さない）**:
    ...
    - **経路A/Bを判定する前に、ローカルの `$headBranch` が `origin/$headBranch` より進んでいないか確認する**（前回の反復で最終pushが失敗し、コミットだけがローカルに残っている可能性がある。ローカルブランチの参照はworktreeを破棄しても残るため、前回そのpush失敗の直後に `ExitWorktree` していても問題なく検出できる）。進んでいる場合、新しい指摘の処理や経路Bの `-B` によるリセット（下記）に進む**前に**、`git push origin $headBranch:$headBranch`（**リポジトリ内のどのディレクトリから実行しても対象を一意にするため、必ず `<ローカル>:<リモート>` の完全な refspec を明示する**。単なる `git push` は「現在チェックアウトしているブランチ」に依存するため、別のworktreeや別ブランチから実行すると意図しない対象に push してしまう）を再試行する。成功したら、そのコミットメッセージに埋め込まれた指摘のCodexコメントID（下記「pushの実行」参照）に 👀 を付けてから、この反復の新しい指摘の処理に進む。**再試行しても失敗する場合は、経路Bの `-B` によるリセットを絶対に実行しない**（`-B` はローカルブランチを強制的に `origin/$headBranch` へ巻き戻すため、まだpushできていない承認済みコミットが跡形もなく失われ、対応するCodexコメントは処理済みのままレビュー不能になる）。新しい指摘の処理はせず、失敗を報告して120秒後にScheduleWakeup
